...
type MapNodeModulesOptionsOmitPolicy = Partial<{
  ...
  /**
   * Additional package locations to graph alongside the entry package.
   * Each entry is graphed into the same compartment map, with an edge from
   * the entry package. If `modules` is specified, those paths are injected
   * into the package's external aliases.
   */
...
export interface AdditionalLocation {
  ...
  /**
   * Specific module paths to expose as external aliases. These paths are
   * relative to the package root (e.g., `'./webpack.config.js'`).
   *
   * If omitted, only modules discovered by standard export inference
   * (`main`, `module`, `exports`, `browser`) are externally visible.
   */
  modules?: string[];
}
